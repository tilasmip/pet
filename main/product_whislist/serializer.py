...
from main.models import ProductWhislist, Category,Breed, ProductWhislist,ProductWhislist, Product
...
class SaveProductWhislistSerializer(serializers.ModelSerializer):
    ...
    def validate(self,attrs):
        product = ProductWhislist.objects.filter(Q(product = attrs.get('product'))&Q(user = attrs.get('user'))).first()
        if product is not None:
            raise serializers.ValidationError({'msg':{'Product already in whislist.'}})
        return attrs
...
class DeleteProductWhislistSerializer(serializers.Serializer):
    ...
    def validate(self,attrs):
        ...
        try:
            self.product_whislist = ProductWhislist.objects.get(Q(id=id)&Q(user =self.context.get('user')))
            if self.product_whislist is None:
                raise serializers.ValidationError({'msg':'ProductWhislist doesn\'t exist.'})
        except:
            raise serializers.ValidationError({'msg':'ProductWhislist doesn\'t exist.'})
        return attrs
